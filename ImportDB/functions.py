...
def balace_change_send_message(address,timestamp,asset,num,phone,code):
    ...
    req.extend = ""
    req.sms_type = "normal"
    req.sms_free_sign_name = "小蚁区块浏览器"
    param={'address':address,'time':timestamp,'asset':asset,'num':num}
    s=json.dumps(param)
    req.sms_param =s #"{address:address,time:'2017.5.21',asset:'anancdnads',num:'111.2222221'}"
    req.rec_num = str(phone)
    if code==1:
        req.sms_template_code = "SMS_67780136"
    else:
        req.sms_template_code = "SMS_67645171"
